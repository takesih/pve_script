...
get_size_configuration() {
    ...
    if [[ "$PVE_LVM_RESIZE_DETECTED" == "true" ]]; then
        ...
        echo ""
        echo "  Root volume will be shrunk and LVM-thin data volume will be created."
...
resize_root_volume() {
    ...
    if (( $(echo "$current_root_size > $target_root_size"| bc -l) )); then
        ...
        if (( $(echo "$current_usage_gb > $target_root_size - 3"| bc -l) )); then
            ...
            echo "  Proceeding with insufficient space - HIGH RISK!"
        fi
        ...
        if [[ "$PVE_LVM_RESIZE_DETECTED" == "true" ]]; then
            ...
            # For initial state systems, provide options for immediate or boot-time resize
            echo "Root filesystem shrinking options:"
            echo "1. Immediate resize (if possible) - Recommended for initial state"
            echo "2. Schedule automatic resize on next boot"
            echo "3. Manual offline operation (Advanced users)"
            ...
            case $shrink_option in
                1)
                    echo "Attempting immediate root volume resize..."
                    
                    # Check if we can safely shrink online
                    if (( $(echo "$current_usage_gb < $target_root_size - 5"| bc -l) )); then
                        echo "✅ Safe to shrink online. Proceeding with immediate resize..."
                        ...
                        if resize2fs /dev/pve/root "$ROOT_SIZE_CALC"; then
                            echo "✅ Filesystem shrunk successfully"
                            
                            # Then shrink logical volume
                            echo "Shrinking logical volume to $ROOT_SIZE_CALC..."
                            if lvresize -L "$ROOT_SIZE_CALC"/dev/pve/root -y; then
                                echo "✅ Logical volume shrunk successfully"
                                
                                # Restart services
                                echo "Restarting Proxmox services..."
                                systemctl start pve-cluster pvedaemon pveproxy || true
                                
                                echo "✅ Immediate root volume resize completed successfully!"
                                return 0
                            else
                                echo "❌ Logical volume shrink failed"
                                echo "Attempting to restore filesystem..."
                                resize2fs /dev/pve/root || true
                                systemctl start pve-cluster pvedaemon pveproxy || true
                                exit 1
                            fi
                        else
                            echo "❌ Filesystem shrink failed"
                            systemctl start pve-cluster pvedaemon pveproxy || true
                            exit 1
                        fi
                    else
                        echo "⚠️  Not safe to shrink online (insufficient free space)"
                        echo "  Current usage: ${current_usage_gb}GB"
                        echo "  Target size: ${target_root_size}GB"
                        echo "  Need at least 5GB free space for safe online shrink"
                        echo ""
                        echo "Scheduling boot-time resize instead..."
                        schedule_boot_resize
                        return 0
                    fi
                    ;;
                ...
                4)
                    ...
                    ;;
                *)
                    echo "Invalid option. Attempting immediate resize..."
                    ...
                    if (( $(echo "$current_usage_gb < $target_root_size - 5"| bc -l) )); then
                        ...
                        echo "⚠️  Not safe to shrink online. Scheduling boot-time resize..."
                        schedule_boot_resize
                        return 0
                    fi
                    ;;
...
STORAGE_EOF
    ...
    if pvesm status --storage local-lvm >/dev/null 2>&1; then
        ...
        echo "  LVM-thin storage may need manual configuration in Proxmox web interface"
    fi
}
